...
## Implementation notes (captured during the Phase 1 build)

Appended as findings surfaced during implementation. Read before starting
any Phase 1b / Phase 2 work that touches these files.

### Autoname gotchas — don't trust record names for upstream submittables

- `Leave Policy` autoname is `HR-LPOL-.YYYY.-.#####` (Expression, old-style).
  The human label sits on the `title` field — the record name is
  auto-generated like `HR-LPOL-2026-00001`.
- `Leave Period` autoname is `HR-LPR-.YYYY.-.#####` — same pattern.
- Any seeder that wants to "assert record X exists" MUST key off a stable
  field (`title` for Leave Policy, `(company, from_date)` for Leave Period).
- Downstream code linking to the record must resolve the autogenerated name
  first via a helper like `_resolve_sa_standard_leave_policy()`. Never
  hardcode `"SA Standard Leave Policy"` as a Link value — it's the title,
  not the name.
- Same lesson likely applies to other v16 HRMS submittables; grep for
  `autoname.*YYYY` before writing any new seeder.

### Settings seeder: hot-path callers should cache-read, not reload

- `install_sa_leave_settings_defaults` uses `frappe.get_single` (writes
  back). Runtime consumers (the `Employee.after_insert` hook, scheduler
  tasks, bulk helpers) use `frappe.get_cached_doc("SA Leave Settings")` so
  each Employee save isn't a full Single re-load. If a HR edit doesn't
  appear to take effect for the hook, it's a cache issue — `bench
  clear-cache` resolves it.

### `Employee.after_insert` must never raise

- Wrapped in `try/except Exception` → `frappe.log_error`. Any guard that
  skips records via `add_comment` on the Employee — NEVER via `frappe.throw`
  — since a throw would abort the Employee insert transaction. The test
  suite confirms this with `test_employee_without_doj_skipped_with_comment`
  and `test_non_sa_employee_is_skipped`.

### Short-tenure window threshold is 90 days, not zero

- Joining-date tail of a cycle can produce a 2-week window which then
  rounds to ~0 Leave Allocation days, resulting in an apparently-broken
  LPA. The hook skips with a Comment when `to_date - effective_from < 90`;
  HR is expected to create the allocation manually on the next cycle.

### Bulk helpers return a uniform shape

- `{"created": N, "skipped": N, "failed": [str]}` — the SA Leave Settings
  `.js` renders one consistent toast. Any new bulk helper added later must
  honour this shape or the toast logic breaks.

### Auto Email Report is still blocked

- `Auto Email Report.email_to` is a hardcoded comma-separated list (no role
  resolution at send time — confirmed via `auto_email_report.py:85-96`).
  Our scheduler tasks work around this by calling `frappe.sendmail` with
  role-resolved recipient lists. Don't try to seed Auto Email Report
  records until this upstream gap is addressed (either by a patch upstream
  or a dedicated wrapper doctype in hrms_za).

### Feb 29 cycle anchor normalisation

- `SALeaveSettings.validate` allows `cycle_start_day = 29` with
  `cycle_start_month = 2`. `_current_cycle_window_for_date` normalises Feb
  29 → Feb 28 in non-leap years via `try/except ValueError`. The same
  normalisation is reapplied inside `seed_leave_period` — keep them in
  sync if either is refactored.

### `send_welcome_email = 0` on provisioned Users

- `provision_employee_users` creates Users with `send_welcome_email = 0`
  deliberately. Sending welcome emails from the installer would spam every
  Employee in one batch the moment HR clicks the button. Instead, HR
  controls invites by flipping `Send Welcome Email` on each User form when
  they're ready — matches how Frappe's normal invite flow behaves.

### Tests are `FrappeTestCase`, not pure pytest

- `test_leave_setup.py` requires a Frappe runtime (creates Companies /
  Employees / Leave Periods). Runs via `bench run-tests --module`. The
  existing `test_paye_calculator.py` is pure pytest — both styles coexist
  in `payroll_sa/tests/`, run separately.

### Shared helper between hook and patch

- `_try_assign_default_policy` is called from both the `Employee.after_insert`
  hook AND the `v0_0_2.backfill_sa_leave_policy_assignments` patch. When
  editing guard logic, both paths move together — NEVER duplicate this into
  two implementations. The patch verifies the function call created an
  assignment by comparing pre/post counts (since the helper may short-circuit
  via any of 9 guards).

### Teardown scope is app-config, not tenant data

- `before_uninstall` removes Notifications + Custom DocPerms. It does NOT
  remove Leave Policy Assignments, Leave Allocations, Leave Periods, the
  Leave Policy itself, or custom fields. Those carry tenant data (employee
  history, custom-field values) that must survive reinstall. If future
  work needs a "hard uninstall" variant, add it as a separate
  whitelisted-method button, never merge it into this teardown path.
