...
from hrms_za.regional.south_africa.data.income_tax_slab import INCOME_TAX_SLAB
...
def setup_per_company(company):
    install_income_tax_slab(company)


def install_income_tax_slab(company):
    """
    Create the 2025/26 SA tax slab as a reference record, disabled by default.
    User enables manually after verification (see data/income_tax_slab.py header).
    """
    payload = dict(INCOME_TAX_SLAB)
    slabs = payload.pop("slabs")
    name = f"{payload['name']} - {company}"

    if frappe.db.exists("Income Tax Slab", name):
        return  # never overwrite — user may have enabled or tweaked
