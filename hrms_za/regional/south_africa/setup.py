...
def resolve_sa_standard_leave_policy():
    """
    Return the autogenerated record name of the submitted SA Standard Leave
    Policy, or None if it hasn't been seeded.

    Callers that link to the policy must use this — never hardcode
    "SA Standard Leave Policy" as a record name (it's the title, not the
    name; Leave Policy autoname is HR-LPOL-.YYYY.-.#####).
    """
...
def install_sa_leave_settings_defaults():
    ...
    if not settings.get("default_leave_policy"):
        resolved = resolve_sa_standard_leave_policy()
        if resolved:
...
def install_leave_period_for_current_year(company):
    ...
    """
    from_date, to_date = current_cycle_window_for_date(frappe.utils.today())
...
def current_cycle_window_for_date(reference_date):
    ...
    If reference_date is on or after the cycle anchor in its own year, the
    window runs from that anchor to one day before the next anchor. Otherwise
    the cycle started in the previous year.
    """
    ...
    current_anchor = anchor_date(ref.year, month, day)
    if ref >= current_anchor:
        return current_anchor, add_days(anchor_date(ref.year + 1, month, day), -1)
    return anchor_date(ref.year - 1, month, day), add_days(current_anchor, -1)
